...
<template>
  <div class="table flex-display flex-column">
    ...
    :row-selection="rowSelection" :rowClassName="(record, index) => ((index % 2) === 0 ? 'table-striped' : null)" :scroll="{ x: '100%', y: 600 }" :locale="{ emptyText: t('common.noData') }">
...
const columns = computed(() => [
  { title: t('members.account'), dataIndex: 'username', width: 150, sorter: (a: Member, b: Member) => a.username.localeCompare(b.username), className: 'titleStyle' },
  { title: t('members.userType'), dataIndex: 'user_type', width: 150, className: 'titleStyle' },
  { title: t('members.workspaceName'), dataIndex: 'workspace_name', width: 150, className: 'titleStyle' },
  { title: t('members.mqttUsername'), dataIndex: 'mqtt_username', width: 150, className: 'titleStyle', slots: { customRender: 'mqtt_username' } },
  { title: t('members.mqttPassword'), dataIndex: 'mqtt_password', width: 150, className: 'titleStyle', slots: { customRender: 'mqtt_password' } },
  { title: t('members.joined'), dataIndex: 'create_time', width: 150, sorter: (a: Member, b: Member) => a.create_time.localeCompare(b.create_time), className: 'titleStyle' },
  { title: t('members.action'), dataIndex: 'action', width: 100, className: 'titleStyle', slots: { customRender: 'action' } },
])
