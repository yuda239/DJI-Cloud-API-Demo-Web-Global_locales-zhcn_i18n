...
<template>
  <div class="ml20 mt20 mr20 flex-row flex-align-center flex-justify-between">
    <div class="flex-row">
      <a-button type="primary" @click="sVisible = true">
        {{ t('firmwares.clickToUpload') }}
      </a-button>
      <a-modal :visible="sVisible"
         :title="t('firmwares.importFirmware')"
         :closable="false"
         ...
         <a-form :rules="rules" ref="formRef" :model="uploadParam" :label-col="{ span: 6 }">
          <a-form-item name="status" :label="t('firmwares.available')" required>
            <a-switch v-model:checked="uploadParam.status" />
          </a-form-item>
          <a-form-item name="device_name" :label="t('firmwares.deviceName')" required>
            <a-select
              style="width: 220px"
              mode="multiple"
              :placeholder="t('firmwares.canChooseMultiple')"
          ...
          <a-form-item name="release_note" :label="t('firmwares.releaseNote')" required>
            <a-textarea v-model:value="uploadParam.release_note" showCount :maxlength="300" />
          </a-form-item>
          <a-form-item :label="t('firmwares.file')" required>
            <a-upload
              :multiple="false"
    ...
    <div class="flex-row">
      <div class="ml5">
        <a-select
          ...
          <a-select-option
            v-for="option in firmwareStatusOptions"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
      ...
      <div class="ml5">
        <a-input-search
          ...
          :placeholder="t('firmwares.searchVersion')"
          style="width: 250px"
  ...
  <div class="table flex-display flex-column">
    <a-table :columns="columns" :data-source="data.firmware" :pagination="paginationProp" @change="refreshData" row-key="firmware_id"
     :rowClassName="(record, index) => ((index % 2) === 0 ? 'table-striped' : null)" :scroll="{ x: '100%', y: 600 }" :locale="{ emptyText: t('common.noData') }">
...
import { onMounted, reactive, Ref, ref, UnwrapRef, computed } from 'vue'
...
const columns = computed(() => [
  { title: t('firmwares.model'), dataIndex: 'device_name', width: 120, ellipsis: true, className: 'titleStyle', slots: { customRender: 'device_name' } },
  { title: t('firmwares.fileName'), dataIndex: 'file_name', width: 220, ellipsis: true, className: 'titleStyle', slots: { customRender: 'file_name' } },
  { title: t('firmwares.firmwareVersion'), dataIndex: 'product_version', width: 180, className: 'titleStyle' },
  { title: t('firmwares.fileSize'), dataIndex: 'file_size', width: 150, className: 'titleStyle', slots: { customRender: 'file_size' } },
  { title: t('firmwares.creator'), dataIndex: 'username', width: 100, className: 'titleStyle' },
  { title: t('firmwares.releaseDate'), dataIndex: 'released_time', width: 160, sorter: (a: Firmware, b: Firmware) => a.released_time.localeCompare(b.released_time), className: 'titleStyle' },
  { title: t('firmwares.releaseNote'), dataIndex: 'release_note', width: 300, ellipsis: true, className: 'titleStyle', slots: { customRender: 'release_note' } },
  { title: t('firmwares.status'), dataIndex: 'firmware_status', width: 100, className: 'titleStyle', slots: { customRender: 'firmware_status' } },
])
...
const deviceNameList = ref<any[]>([{ label: t('firmwares.all'), value: '' }])

const firmwareStatusOptions = computed(() => [
  { label: t('firmwares.all'), value: FirmwareStatusEnum.NONE },
  { label: t('firmwares.disabled'), value: FirmwareStatusEnum.FALSE },
  { label: t('firmwares.available'), value: FirmwareStatusEnum.TRUE }
])
...
const rules = {
  ...
  release_note: [{ required: true, message: t('firmwares.releaseNoteRequired') }],
  device_name: [{ required: true, message: t('firmwares.selectDeviceModels') }]
}
interface FileItem {
  uid: string;
  name?: string;
