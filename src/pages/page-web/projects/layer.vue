<template>
  <div class="project-layer-wrapper height-100">
    <div style="height: 50px; line-height: 50px; border-bottom: 1px solid #4f4f4f; font-weight: 450;">
      <a-row>
        <a-col :span="1"></a-col>
        <a-col :span="22">{{ t('layer.annotations') }}</a-col>
        <a-col :span="1"></a-col>
      </a-row>
    </div>
    ...
    >
      <div class="drawer-element-content">
        <div class="name element-item">
          <span class="title">{{ t('layer.name') }}:</span>
          <a-input
            v-model:value="layerState.layerName"
            style="width:120px"
            :placeholder="t('layer.elementName')"
            @change="changeLayer"
          />
        </div>
        <div
        ...
        >
          <span class="title">{{ t('layer.longitude') }}:</span>
          <a-input
            v-model:value="layerState.longitude"
            style="width:120px"
            :placeholder="t('layer.longitude')"
            @change="changeLayer"
          />
        </div>
        <div
          ...
          v-if="layerState.currentType === geoType.Point"
        >
          <span class="title">{{ t('layer.latitude') }}:</span>
          <a-input
            v-model:value="layerState.latitude"
            style="width:120px"
            :placeholder="t('layer.latitude')"
            @change="changeLayer"
          />
        </div>
        <div class="color-content">
          <span class="mr30">{{ t('layer.color') }}: </span>
          <div
      ...
      <div class="flex-row flex-justify-around flex-align-center mt20">
        <a-button type="primary" @click="deleteElement">{{ t('layer.delete') }}</a-button>
      </div>
    </a-drawer>
  </div>
</template>
