...
<template>
  ...
  <div class="device-table-wrap table flex-display flex-column">
    ...
    :row-selection="rowSelection" :rowClassName="rowClassName" :scroll="{ x: '100%', y: 600 }"
      :expandIcon="expandIcon" :loading="loading" :locale="{ emptyText: t('common.noData') }">
      ...
      <template #action="{ record }">
        <div class="editable-row-operations">
          ...
          <div v-if="editableData[record.device_sn]">
            <a-tooltip :title="t('common.confirm')">
          ...
          <div v-else class="flex-align-center flex-row" style="color: #2d8cf0">
            <a-tooltip v-if="current.indexOf(EDeviceTypeName.Dock) !== -1" :title="t('devices.deviceLog')">
              <CloudServerOutlined @click="showDeviceLogUploadRecord(record)"/>
            </a-tooltip>
            <a-tooltip v-if="current.indexOf(EDeviceTypeName.Dock) !== -1" :title="t('devices.hmsInfo')">
    ...
    <a-modal v-model:visible="deleteTip" width="450px" :closable="false" centered :okButtonProps="{ danger: true }" @ok="unbind">
        <p class="pt10 pl20" style="height: 50px;">{{ t('devices.deleteConfirm') }}</p>
        <template #title>
            <div class="flex-row flex-justify-center">
                <span>{{ t('devices.deleteDevice') }}</span>
            </div>
        </template>
    </a-modal>

    <!-- 设备升级 -->
    <DeviceFirmwareUpgradeModal :title="t('devices.deviceUpgrade')"
...
import { h, onMounted, reactive, ref, UnwrapRef, computed } from 'vue'
...
const columns = computed(() => [
  { title: t('devices.model'), dataIndex: 'device_name', width: 100, className: 'titleStyle' },
  { title: t('devices.sn'), dataIndex: 'device_sn', width: 100, className: 'titleStyle', ellipsis: true, slots: { customRender: 'sn' } },
  {
  ...
  { title: t('devices.firmwareVersion'), dataIndex: 'firmware_version', width: 150, className: 'titleStyle', slots: { customRender: 'firmware_version' } },
  { title: t('devices.status'), dataIndex: 'status', width: 100, className: 'titleStyle', slots: { customRender: 'status' } },
  {
    title: t('devices.workspace'),
  ...
  { title: t('devices.joined'), dataIndex: 'bound_time', width: 150, sorter: (a: Device, b: Device) => a.bound_time.localeCompare(b.bound_time), className: 'titleStyle' },
  { title: t('devices.lastOnline'), dataIndex: 'login_time', width: 150, sorter: (a: Device, b: Device) => a.login_time.localeCompare(b.login_time), className: 'titleStyle' },
  {
