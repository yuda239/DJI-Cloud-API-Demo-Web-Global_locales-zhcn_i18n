<template>
    ...
    >
      <p class="ml10 mb0 fz16">{{ t('livestream.selectLivestreamType') }}:</p>
      <a-select
        style="width: 200px; margin-right: 20px;"
        :placeholder="t('livestream.selectLiveType')"
    ...
    <div class="width-100" style="margin-top: -10px;">
      <div class="ml10" style="width: 97%;">
        <span class="fz16">{{ t('livestream.param') }}: </span>
        <span v-if="liveStreamStatus.type === ELiveTypeValue.Agora" style="word-break: break-all; color: #75c5f6;">
          <div class="flex-col flex-justify-center flex-align-center">
            <div>
              <span class="ml10">{{ t('livestream.token') }}:</span>
              <a-input
                class="ml10"
                v-model:value="agoraParam.token"
                :placeholder="t('livestream.token')"
              ></a-input>
            </div>
            <div>
              <span class="ml10">{{ t('livestream.channel') }}:</span>
              <a-input
                class="ml10"
                v-model:value="agoraParam.channelId"
                :placeholder="t('livestream.channel')"
              ></a-input>
            </div>
          </div>
        </span>
    ...
    <div class="mb20 flex-row flex-align-center flex-justify-center"
      style="width: 100%; ">
      <a-button class="flex-column fz20 flex-align-center flex-justify-center" style="width: 100px;" type="ghost" @click="onPlay">{{ t('livestream.play') }}</a-button>
      <a-button class="flex-column fz20 flex-align-center flex-justify-center ml40" style="width: 100px;" type="ghost" @click="onStop">{{ t('livestream.stop') }}</a-button>
    </div>
...
const onPlay = () => {
  ...
  if (!publishModeSelected.value) {
    message.warn(t('livestream.pleaseSelectPublishMode'))
    return
  }
  if (liveTypeSelected.value === ELiveTypeName.Unknown) {
    message.warn(t('livestream.pleaseSelectLivestreamType'))
    return
  }
  switch (liveStreamStatus.type) {
    case 1: {
