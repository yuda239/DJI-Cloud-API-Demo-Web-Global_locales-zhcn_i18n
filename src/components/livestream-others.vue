<template>
  <div class="flex-column flex-justify-start flex-align-center">
    <video
      ...
      class="mt20"
    ></video>
    <p class="fz24">{{ t('livestream.liveStreamingSourceSelection') }}</p>

    <div class="flex-row flex-justify-center flex-align-center mt10">
      <template v-if="liveState && isDockLive">
        <span class="mr10">{{ t('livestream.lens') }}:</span>
      ...
      <a-select
        class="ml10"
        style="width:150px"
        :placeholder="t('livestream.selectDrone')"
      ...
      <a-select
        class="ml10"
        style="width:150px"
        :placeholder="t('livestream.selectCamera')"
      ...
      <a-select
        class="ml10"
        style="width:150px"
        :placeholder="t('livestream.selectClarity')"
    ...
    <div class="mt20">
      <p class="fz10" v-if="livetypeSelected == 2">
        {{ t('livestream.rtspNote') }}
      </p>
      <p class="fz10" v-if="livetypeSelected == 2">
        {{ t('livestream.rtspParameter') }}:{{ rtspData }}
      </p>
    </div>
    <div class="mt10 flex-row flex-justify-center flex-align-center">
      <a-button v-if="liveState && isDockLive" type="primary" large @click="onSwitch">{{ t('livestream.switchLens') }}</a-button>
      <a-button v-else type="primary" large @click="onStart">{{ t('livestream.play') }}</a-button>
      <a-button class="ml20" type="primary" large @click="onStop"
        >{{ t('livestream.stop') }}</a-button
      >
      <a-button class="ml20" type="primary" large @click="onUpdateQuality"
        >{{ t('livestream.updateClarity') }}</a-button
      >
      <a-button v-if="!liveState || !isDockLive" class="ml20" type="primary" large @click="onRefresh"
        >{{ t('livestream.refreshLiveCapacity') }}</a-button
      >
    </div>
  </div>
</template>
