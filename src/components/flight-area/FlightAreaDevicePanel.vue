<template>
  <div class="flight-area-device-panel">
    <Title :title="t('flightArea.chooseSyncDevices')">
    ...
    <div class="scrollbar">
      <div id="data" v-if="data.length !== 0">
              ...
              <div class="ml10 mr10 pr5 pl5 flex-align-center flex-row flex-justify-between" style="background: #595959;">
                ...
                <div>
                  ...
                  <div v-else-if="deviceStatusMap[dock.device_sn]?.flight_area_status?.sync_status === ESyncStatus.SYNCHRONIZING
                    ...
                    <a-tooltip :title="t('flightArea.toBeSynced')">
                      <SyncOutlined spin />
                    </a-tooltip>
                  </div>
                  <div v-else>
                    <a-tooltip :title="deviceStatusMap[dock.device_sn]?.flight_area_status?.sync_msg || t('flightArea.noSynchronization')">
        ...
        <div class="flex-row flex-justify-between footer">
          <a-button class="mr10" @click="closePanel">{{ t('common.cancel') }}
          </a-button>
          <a-button type="primary" :disabled="confirmDisabled" @click="syncDeviceFlightArea">{{ t('flightArea.sync') }}
          </a-button>
        </div>
      </div>
      <div v-else>
...
const selectDock = (dock: Device) => {
  if (!dock.status) {
    message.info(t('flightArea.dockOfflineMessage', { nickname: dock.nickname }))
    return
  }
  if (deviceStatusMap.value[dock.device_sn]?.flight_area_status?.sync_status === ESyncStatus.SYNCHRONIZING ||
    ...
    message.info(t('flightArea.dockSynchronizing'))
    return
  }
...
const syncDeviceFlightArea = () => {
  ...
  if (keys.length === 0) {
    message.warn(t('flightArea.selectDocksToSync'))
    return
  }
  ...
  syncFlightArea(keys).then(res => {
    if (res.code === 0) {
      message.success(t('flightArea.devicesSynchronizing'))
