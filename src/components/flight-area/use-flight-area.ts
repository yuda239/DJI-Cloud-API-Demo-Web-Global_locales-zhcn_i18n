...
export function useFlightArea () {
  ...
  function checkCircle (obj: any): boolean {
    if (obj.getRadius() < MIN_RADIUS) {
      message.error(t('flightArea.radiusError', { minRadius: MIN_RADIUS }))
      root.$map.remove(obj)
      return false
    }
    return true
  ...
  function checkPolygon (obj: any): boolean {
    ...
    if (path.length < 3) {
      message.error(t('flightArea.polygonPathError'))
      root.$map.remove(obj)
      return false
    }
  ...
  const onFlightAreaDroneLocationWs = (data: CommonHostWs<FlightAreasDroneLocation>) => {
    ...
    notification.warning({
      ...
      message: t('flightArea.droneFlightAreaInfo', { sn: data.sn }),
      description: h('div',
        [
          h('div', [
            h('span', { class: 'fz18' }, t('flightArea.inFlightArea') + ': '),
            h('ul', [
              ...inArea.map(val => h('li', t('flightArea.distanceFromArea', { distance: val.area_distance, areaName: coverMap[val.area_id][1]?.getText() || val.area_id })))
            ])
          ]),
          h('div', [
            h('span', { class: 'fz18' }, t('flightArea.nearFlightArea') + ': '),
            h('ul', [
              ...nearArea.map(val => h('li', t('flightArea.distanceFromArea', { distance: val.area_distance, areaName: coverMap[val.area_id][1]?.getText() || val.area_id })))
            ])
          ])
        ]),
      duration: null,
