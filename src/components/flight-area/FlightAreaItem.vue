<template>
  <div class="panel" style="padding-top: 5px;" :class="{disable: !flightArea.status}">
    ...
    <div class="mt5 ml10" style="color: hsla(0,0%,100%,0.35);">
      <span class="mr10">{{ t('flightArea.updateAt') }} {{ formatDateTime(flightArea.update_time).toLocaleString() }}</span>
    </div>
    <div class="flex-row flex-justify-between flex-align-center ml10 mt5" style="color: hsla(0,0%,100%,0.65);">
      ...
      <div class="mr10 operate">
        <a-popconfirm v-if="flightArea.status" :title="t('flightArea.disableConfirm')" :okText="t('flightArea.disable')" @confirm="changeAreaStatus(false)">
          <stop-outlined />
        </a-popconfirm>
        <a-popconfirm v-else @confirm="changeAreaStatus(true)" :title="t('flightArea.enableConfirm')" :okText="t('flightArea.enable')" >
          <check-circle-outlined />
        </a-popconfirm>
        <EnvironmentFilled class="ml10" @click="clickLocation"/>
        <a-popconfirm :title="t('flightArea.deleteConfirm')" :okText="t('common.delete')" okType="danger" @confirm="deleteArea">
