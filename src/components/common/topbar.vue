  ...
  <div class="width-100 flex-row flex-justify-between flex-align-center" style="height: 60px;">
    ...
    <div class="height-100 fz16 flex-row flex-justify-between flex-align-center">
      ...
      <a-dropdown class="mr20">
        <div class="height-100 flex-row flex-align-center cursor-pointer" style="color: white;">
          <GlobalOutlined class="mr5" />
          <span>{{ currentLocale === 'zh' ? '中文' : 'English' }}</span>
        </div>
        <template #overlay>
          <a-menu theme="dark">
            <a-menu-item @click="switchLanguage('zh')">
              <span>中文</span>
            </a-menu-item>
            <a-menu-item @click="switchLanguage('en')">
              <span>English</span>
            </a-menu-item>
          </a-menu>
        </template>
        ...
        <template #overlay>
          <a-menu theme="dark" class="flex-column flex-justify-between flex-align-center">
            <a-menu-item>
              ...
              <span @click="logout">{{ t('nav.logout') }}</span>
            </a-menu-item>
          </a-menu>
        </template>
      </a-dropdown>
...
import { UserOutlined, ExportOutlined, GlobalOutlined } from '@ant-design/icons-vue'
...
const options = computed(() => [
  { key: 0, label: t('nav.workspace'), path: '/' + ERouterName.WORKSPACE },
  { key: 1, label: t('nav.members'), path: '/' + ERouterName.MEMBERS },
  { key: 2, label: t('nav.devices'), path: '/' + ERouterName.DEVICES },
  { key: 3, label: t('nav.firmwares'), path: '/' + ERouterName.FIRMWARES },
])
...
const switchLanguage = (locale: 'zh' | 'en') => {
  ...
  message.success(locale === 'zh' ? '语言已切换为中文' : 'Language switched to English')
}
</script>
