<template>
  <div class="header">{{ t('tasks.title') }}</div>
     ...
     <!-- 媒体上传状态 -->
      ...
      <template #action="{ record }">
        <div class="action-area">
          ...
          >
            <a-button type="primary" size="small">{{ t('common.delete') }}</a-button>
          </a-popconfirm>
          <a-popconfirm
            v-if="record.status === TaskStatus.Carrying"
            :title="t('task.suspendConfirm')"
            :ok-text="t('common.yes')"
            :cancel-text="t('common.no')"
            @confirm="onSuspendTask(record.job_id)"
          >
            <a-button type="primary" size="small">{{ t('task.suspend') }}</a-button>
          </a-popconfirm>
          <a-popconfirm
            v-if="record.status === TaskStatus.Paused"
            :title="t('task.resumeConfirm')"
            :ok-text="t('common.yes')"
            :cancel-text="t('common.no')"
            @confirm="onResumeTask(record.job_id)"
          >
            <a-button type="primary" size="small">{{ t('task.resume') }}</a-button>
          </a-popconfirm>
        </div>
      </template>
    </a-table>
...
const columns = [
  {
    title: t('tasks.plannedActualTime'),
    dataIndex: 'duration',
    width: 200,
  ...
  {
    title: t('tasks.flightRouteName'),
    dataIndex: 'file_name',
    width: 100,
  },
  {
    title: t('tasks.dockName'),
    dataIndex: 'dock_name',
    width: 100,
    ellipsis: true
  },
  {
    title: t('tasks.rthAltitude'),
    dataIndex: 'rth_altitude',
    width: 120,
  },
  {
    title: t('tasks.lostAction'),
  ...
  {
    title: t('tasks.mediaFileUpload'),
    key: 'media_upload',
    width: 160,
    slots: { customRender: 'media_upload' }
  },
  {
    title: t('tasks.action'),
    width: 120,
...
async function onSuspendTask (jobId: string) {
  ...
  if (code === 0) {
    message.success(t('task.suspendSuccess'))
    getPlans()
  }
}
...
async function onResumeTask (jobId: string) {
  ...
  if (code === 0) {
    message.success(t('task.resumeSuccess'))
    getPlans()
  }
}
...
async function onUploadMediaFileNow (jobId: string) {
  ...
  if (code === 0) {
    message.success(t('task.uploadMediaSuccess'))
    getPlans()
  }
}
</script>
