  ...
  <div class="create-plan-wrapper">
    ...
    <div class="content">
      <a-form ref="valueRef" layout="horizontal" :hideRequiredMark="true" :rules="rules" :model="planBody" labelAlign="left">
        ...
        <!-- 任务类型 -->
        <a-form-item :label="t('tasks.planTimer')" class="plan-timer-form-item">
        ...
        <!-- execute date -->
        <a-form-item :label="t('tasks.date')" v-if="planBody.task_type === TaskType.Timed || planBody.task_type === TaskType.Condition" name="select_execute_date" :labelCol="{span: 23}">
          <a-range-picker
        ...
        <!-- execute time -->
        <a-form-item :label="t('tasks.time')" v-if="planBody.task_type === TaskType.Timed || planBody.task_type === TaskType.Condition"
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          <!-- battery capacity -->
          <a-form-item :label="t('tasks.startTaskWhenBatteryReaches')" :labelCol="{span: 23}" name="min_battery_capacity">
  ...
  select_execute_time: [{
    validator: async (rule: RuleObject, value: Moment[]) => {
      ...
      if (planBody.select_time.length < planBody.select_time_number) {
        throw new Error(t('tasks.selectTime'))
      }
      validOverlapped()
    }
  }],
  select_execute_date: [{ required: true, message: t('tasks.selectDate') }],
  rth_altitude: [
    {
...
function validStartTime (): Error | void {
  for (let i = 0; i < planBody.select_time.length; i++) {
    if (!planBody.select_time[i][0]) {
      throw new Error(t('tasks.selectStartTime'))
    }
  }
}
...
function validOverlapped (): Error | void {
  ...
  arr.forEach((v, i, arr) => {
    if (i > 0 && v[0] < arr[i - 1][1]) {
      throw new Error(t('tasks.overlappingTimePeriods'))
    }
  })
}
